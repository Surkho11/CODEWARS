...
function scoreTest(str, right, omit, wrong){
    // Находим все правильные ответы и умножаем на right
    const correctAnswer = str.filter(correct => correct === 0).length * right;
    // Находим все пропушенные ответы и умножаем на omit
    const missingAnswer = str.filter(missing => missing === 1).length * omit;
    // Находим все неправильные ответы и умножаем на wrong
    const incorrectAnswer =str.filter(incorrect => incorrect === 2).length * wrong;

    return correctAnswer + missingAnswer - incorrectAnswer
}
