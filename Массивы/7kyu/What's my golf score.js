...
function golfScoreCalculator(parList, scoreList){
    let sumParList = parList.split('').reduce((acc, num) => acc + Number(num), 0);
    let sumscoreList = scoreList.split('').reduce((acc, num) => acc + Number(num), 0);

    return sumscoreList - sumParList;
}
